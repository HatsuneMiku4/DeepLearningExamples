...
def main(args):
    ...
    directory_structure = {
        ...
        'sharded': working_dir + '/sharded_' + "training_shards_" + str(args.n_training_shards) + "_test_shards_" + str(
            args.n_test_shards) + "_fraction_" + str(args.fraction_test_set),
    ...
    if args.action == 'download':
        ...
        if args.dataset == 'bookscorpus':
            ...
            #
            # assert os.stat(
            #     output_filename).st_size > 0, 'File glob did not pick up extracted wiki files from WikiExtractor.'
        ...
        #         'tfrecord'] + '/' + args.dataset + '/' + filename_prefix + '_' + str(shard_id) + '.' + output_format
